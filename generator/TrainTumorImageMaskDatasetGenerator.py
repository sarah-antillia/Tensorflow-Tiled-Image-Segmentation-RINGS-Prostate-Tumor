...
class ImageMaskDatasetGenerator:
  ...
  def generate(self):
    ...
    for i in range(l1):
      rc = self.generate_files(mask_files[i],   i, self.output_masks_dir, mask=True) 
      if rc:
  ...
  def generate_files(self, image_file, index, output_dir, mask=False):
     ...
     if mask:
       if np.all(image==0):
         print("---Skip all black mask {}".format(image_file))
         return False
     if mask==False:
        if not os.path.exists(mask_file):
          print("---Skip image file {}".format(image_file))
          return False
...
if __name__ == "__main__":
  try:
    output_dir  = "./Prostate-Train-Tumor-512x3-master/"
    ...
    train_masks_dir   = "./RINGS/TRAIN/MANUAL TUMOR/"
    augmentation= False
